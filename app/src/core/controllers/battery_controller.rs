...
impl BatteryController{
    pub async fn bms_can_handle(&mut self, id: u16, data: &[u8], sender:DataSender, timestamp: u64) {
        let own_id = self.id;
        match id {
            x if x == own_id => {
            ...
            x if x <= own_id+32+self.number_of_groups as u16 => {
                self.individual_voltages_bms(data,(x-own_id-32) as u8,timestamp).await;
            }
            x=>{
                self.individual_temperature_bms(data,(x-own_id-256) as u8,timestamp).await;
            }
        }
    }
    pub async fn battery_voltage_overall_bms(&mut self,data: &[u8],timestamp: u64){
        ...
        self.data_sender.send(Datapoint::new(Datatype::BatteryVoltage,(min_cell_voltage as u64) <<32 | (max_cell_voltage as u64) <<16 | avg_cell_voltage as u64,timestamp)).await;
        self.data_sender.send(Datapoint::new(Datatype::TotalBatteryVoltage,total_pack_voltage as u64,timestamp)).await;
    }
    ...
    pub async fn state_of_charge_bms(&mut self,data: &[u8],timestamp: u64){
        ...
        self.data_sender.send(Datapoint::new(Datatype::BatteryCurrent,current as u64,timestamp)).await;
        self.data_sender.send(Datapoint::new(Datatype::ChargeState,(estimated_charge<<8) as u64 | state_of_charge as u64,timestamp)).await;
    }
    ...
    pub async fn overall_temperature_bms(&mut self,data: &[u8],timestamp: u64){
        ...
        self.data_sender.send(Datapoint::new(Datatype::BatteryTemperature,(min_temp as u64) <<16| (max_temp as u64) <<8 | avg_temp as u64,timestamp)).await;
    }

    pub async fn individual_temperature_bms(&mut self,data: &[u8],group_number: u8,timestamp: u64){
       let d = data.iter().for_each(|&x|{ x - 100;}); //CELSIUS base -100 C
        let mut n:u64 = 0;
        for (i, &x) in data.iter().enumerate() {
            self.data_sender.send(Datapoint::new(Datatype::SingleCellTemperature,(group_number as u64)<<16|(n<<8)|x as u64,timestamp)).await;
            n+=1;
        }
    }
    ...
    pub async fn individual_voltages_bms(&mut self,data: &[u8],group_number: u8,timestamp: u64){
        let mut n:u64 = 0;
        for (i, &x) in data.iter().enumerate() {
            self.data_sender.send(Datapoint::new(Datatype::SingleCellVoltage,(group_number as u64)<<16|(n<<8)|x as u64,timestamp)).await;
            n+=1;
        }
    }
    ...
    pub async fn overall_balancing_status_bms(&mut self,data: &[u8],timestamp: u64){
        ...
        self.data_sender.send(Datapoint::new(Datatype::BatteryBalance,(min_cell_balancing as u64) <<16 | (max_cell_balancing as u64) <<8 | avg_cell_balancing as u64,timestamp)).await;
    }
