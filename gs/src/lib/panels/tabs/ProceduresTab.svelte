    ...
    <div>
        <AppRail background="bg-surface-900" aspectRatio="aspect-auto" width="w-50">
            {#each $procedures as procedure, i}
                <AppRailTile regionLabel="text-left px-4" bind:group={currentTile} name={procedure.name} value={i} title={procedure.title}>
                    <p class="py-2 text-base font-medium">{procedure.name}</p>
                </AppRailTile>
            {/each}
        </AppRail>
    </div>
    <div class="p-8 w-10/12 flex-grow flex flex-col gap-4">
        <h2 class="text-2xl font-bold">{$procedures[currentTile].title}</h2>
        ...
        <p>{$procedures[currentTile].content}</p>
    </div>
</div>
