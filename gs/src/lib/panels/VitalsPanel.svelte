<script lang="ts">
    import { Battery, Table, FSM, TileGrid, Tile, CommandButton, Localiser, Store } from '@delft-hyperloop/serpenta';
...
<div bind:clientWidth={width} class="h-full bg-surface-700 text-surface-50">
    <AppBar padding="p-3" border="border-b border-b-surface-900" background="bg-surface-700">
        ...
        <svelte:fragment slot="trail">
            ...
            }} className="bg-error-500 text-surface-100 btn py-0 border border-error-500 rounded-sm"
                           cmd="EmergencyBrake" />
        </svelte:fragment>
    </AppBar>
    ...
    {#if width < 200}
        <div class="flex flex-col h-full pb-20 justify-between items-center">
            <button on:click={() => {
                invoke('send_CommandButton', {cmdName: "EmergencyBrake", val: 0}).then(() => {
            ...
            <div class="flex flex-col gap-4">
                <Battery fill="#3b669c" vertical={true} height={55} percentage={Number($lvBattery.value)} />
                <Battery fill="#723f9c" vertical={true} height={55} percentage={Number($hvBattery.value)} />
            </div>
        </div>
    {:else}
        <div class="snap-x scroll-px-0.5 snap-mandatory overflow-x-auto h-[90vh]">
            <TileGrid className="p-4 w-full" columns="1fr 1fr" rows="">
                ...
                <Tile bgToken={700} containerClass="col-span-2">
                    <div class="flex flex-wrap justify-between">
                        <div class="flex gap-4 ">
                            <p>
                                Velocity:
                                <Store datatype="Velocity" />
                                m/s
                                <br>
                                Position:
                                <Store datatype={LOCALISATION_NAME} />
                                mm
                                <br>
                                Acceleration:
                                <Store datatype="Acceleration" />
                                m/s²
                            </p>
                            <p>
                                HV Current:
                                <Store datatype="BatteryCurrentHigh" />
                                - [0, 25] A
                                <br>
                                LV Current:
                                <Store datatype="BatteryCurrentLow" />
                                - [0, 10] A
                            </p>
                            <p>
                                Low Pressure:
                                <Store datatype="LowPressureSensor" />
                                - [40, 52] bar
                                <br>
                                High Pressure:
                                <Store datatype="HighPressureSensor" />
                                - [80, 180] bar
                            </p>
                        </div>
                        <div style="grid-template-columns: 1fr 2fr 3fr;" class="grid gap-2">
                            <span>LV: </span>
                            <Battery fill="#3b669c" vertical={false} percentage={Number($lvBattery.value)} />
                            <span>Total: <Store datatype="TotalBatteryVoltageLow" /></span>

                            <span>HV: </span>
                            <Battery fill="#723f9c" vertical={false} percentage={Number($hvBattery.value)} />
                            <span>Total: <Store datatype="TotalBatteryVoltageHigh" /></span>
                        </div>
                    </div>
                    <div class="flex flex-wrap justify-between mt-4">
                        <div class="flex gap-4">
                            <CommandButton cmd="StopHV" className="py-1 text-error-400 border-error-400 border-2" />
                            <CommandButton cmd="ArmBrakes" className="py-1 bg-primary-500 text-surface-900 " />
                            <CommandButton cmd="StartRun" className="py-1 bg-primary-500 text-surface-900" />
                            <CommandButton cmd="ContinueRun" className="py-1 bg-primary-500 text-surface-900" />
                        </div>
                ...
                <Tile containerClass="pt-2 pb-1 col-span-2" bgToken={800}>
                    <Table tableArr={tableTempsArr}
                           titles={["Module", "Temp °C", "Safe range", "Module", "Temp °C", "Safe range"]} />
                </Tile>
                <Tile containerClass="pt-2 pb-1 col-span-2" bgToken={800}>
                    <Table titles={["Datatype", "Value", "Safe range", "Datatype", "Value", "Safe range"]}
                           tableArr={tableArr2} />
                </Tile>
